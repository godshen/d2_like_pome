...
async def service_draw_get_one(msg_api: qqbot.AsyncMessageAPI, message: qqbot.Message):
    ...
    send = qqbot.MessageSendRequest("<@%s> 久等啦！您抽中的是第 %d 签\n签诗：%s\n" % (uid, num, draw_poem), message.id)
    await msg_api.post_message(message.channel_id, send)


async def service_draw_solve_one(msg_api: qqbot.AsyncMessageAPI, message: qqbot.Message):
    uid = message.author.id
    uid_date = _get_uid_date(uid)
    num = _get_num_from_cache(uid_date)
    if num is None:
        send = qqbot.MessageSendRequest("<@%s> 你还没有抽签\n" % uid, message.id)
    else:
        draw_explain = dao.get_draw_one_explain(num)
        send = qqbot.MessageSendRequest("<@%s> 解签：%s\n" % (uid, draw_explain), message.id)
    await msg_api.post_message(message.channel_id, send)
